...
def get_file_metadata(file_path):
    ...
    cursor.execute("SELECT file_name, file_path, file_size, file_modified, last_scanned FROM FileRecords WHERE file_path = ?", (file_path,))
...
def store_scan_results(file_name, file_path, file_size, file_modified):
    ...
    cursor.execute('''
        INSERT INTO FileRecords (file_name, file_path, file_size, file_modified, last_scanned)
        VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(file_path) DO UPDATE SET 
            file_size = excluded.file_size,
            file_modified = excluded.file_modified,
            last_scanned = CURRENT_TIMESTAMP
    ''', (file_name, file_path, file_size, file_modified))

    conn.commit()
    conn.close()

    logging.info(f"Updated metadata for file: {file_name}")
